...
(async () => {
  ...
  try {
    ...
    // Find branches matching Baggao
    const branchesRes = await client.query("SELECT id, name FROM branches WHERE name ILIKE '%baggao%'");
    if (branchesRes.rows.length === 0) {
      console.log('No branch found with name matching "baggao"');
      await client.end();
      process.exit(0);
    }
    ...
    for (const br of branchesRes.rows) {
      ...
      // Inventory movements for this branch
      const invRes = await client.query(
        `SELECT im.id, im.item_id, im.date_received, im.remarks, im.created_at,
      ...
      if (invRes.rowCount === 0) {
        ...
        console.table(invRes.rows.map(r => ({
          id: r.id,
          item_id: r.item_id,
          date_received: r.date_received ? r.date_received.toISOString().slice(0,10) : null,
          total_units: Number(r.total_units),
          remarks: r.remarks
        })));
      }

      // Aggregate units for this branch
      ...
      // Show any inventory movements that currently have zero units (maybe empty state)
      const emptyInv = await client.query(
        `SELECT im.id, im.item_id, im.date_received, im.remarks
         FROM inventory_movements im
         WHERE im.branch_id = $1
      ...
      // If total_units is zero, print note
    ...
    console.error('Error checking Baggao inventory:', err);
