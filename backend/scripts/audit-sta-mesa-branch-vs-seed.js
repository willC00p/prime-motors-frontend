...
function parseSeedRows(filePath){
  ...
  for(let p of parts){
    ...
    objs.push({ dr_no: get('dr_no'), engine: get('engine'), chassis: get('chassis'), date: get('date'), model: get('model') });
  }
  return objs;
}
  ...
  try{
    ...
    for(const r of rows){
      ...
      const q = await client.query(`
        ...
        LIMIT 1
      `,[branchId, eng, chs]);
      if(q.rows.length===0){ missingAtBranch.push(r); }
    }

    console.log(JSON.stringify({ totalSeedRows: rows.length, missingAtBranchCount: missingAtBranch.length, missingAtBranch }, null, 2));
  }catch(e){
