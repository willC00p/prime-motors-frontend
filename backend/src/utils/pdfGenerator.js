...
exports.generateLoanReportPDF = generateLoanReportPDF;
exports.generatePOPDF = generatePOPDF;
const pdfkit_1 = __importDefault(require("pdfkit"));
function generateLoanReportPDF(res, data) {
    ...
    const columns = [
        ...
        { header: 'Total Loan Amount', width: 100, align: 'right' },
        { header: 'Monthly Payment', width: 100, align: 'right' },
        { header: 'Amount Paid', width: 100, align: 'right' },
        { header: 'Balance', width: 100, align: 'right' },
        { header: 'Paid/Total', width: 80, align: 'center' },
...
function generatePOPDF(res, data) {
    // Create a new PDF document
    const doc = new pdfkit_1.default({
        margin: 50,
        size: 'A4'
    ...
    columns.forEach((col, index) => {
        let align = 'left';
        if (index === 3 || index === 5)
            align = 'center'; // QTY and REBATE %
        if (index === 4 || index === 6)
            align = 'right'; // UNIT PRICE and AMOUNT
    ...
    doc
        ...
        .font('Helvetica')
        .text('₱' + data.net_amount.toLocaleString(), valuesX, currentY);
    if (data.due_date) {
        currentY += 20;
        doc
            .font('Helvetica-Bold')
