...
function extractAreaFromAddress(address) {
    ...
    for (const [canonical, patterns] of entries) {
        for (const p of patterns) {
            ...
            if (pat && addr.includes(pat))
                return canonical;
        }
    }
    // Fallbacks based on province mention
    if (addr.includes('cagayan'))
        return 'Cagayan (unspecified)';
    if (addr.includes('isabela'))
        return 'Isabela (unspecified)';
    if (addr.includes('metro manila') || addr.includes('ncr'))
        return 'Metro Manila (unspecified)';
    return 'Other Areas';
}
...
function getDashboardAnalytics(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const agentUnitStats = {};
            const fmoStats = {};
            const bmStats = {};
            const mechanicStats = {};
            const baoStats = {};
            // Per-branch, per-month sales (amount) and units
            const branchMonthStats = {};
            const branchMonthUnitStats = {};
            const branchUnitTotals = {};
            ...
            sales.forEach((sale) => {
                ...
                if (sale.agent) {
                    ...
                    for (const it of itemUnits) {
                        if (it.vehicle_unit_id)
                            distinctVehicleIds.add(Number(it.vehicle_unit_id));
                ...
                const branch = ((_a = sale.branches) === null || _a === void 0 ? void 0 : _a.name) || String(sale.branch_id);
                const month = sale.date_sold ? new Date(sale.date_sold).toLocaleString('en-US', { month: 'short', year: 'numeric' }) : 'Unknown';
                if (!branchMonthStats[branch])
                    branchMonthStats[branch] = {};
                ...
                const unitsInSale = (sale.sales_items || []).reduce((sum, it) => sum + Number(it.qty || 1), 0) || 0;
                if (!branchMonthUnitStats[branch])
                    branchMonthUnitStats[branch] = {};
                ...
                (_b = sale.sales_items) === null || _b === void 0 ? void 0 : _b.forEach((item) => {
                    var _a;
                    const model = (_a = item.items) === null || _a === void 0 ? void 0 : _a.model;
                    if (model) {
                        if (!modelStats[model]) {
                            modelStats[model] = {
                                totalAmount: 0,
            ...
            const [{ available } = { available: 0 }] = yield prisma_1.default.$queryRawUnsafe("SELECT COUNT(*)::int AS available FROM vehicle_units WHERE COALESCE(transferred,false)=false AND status='available'");
            const [{ sold } = { sold: 0 }] = yield prisma_1.default.$queryRawUnsafe("SELECT COUNT(*)::int AS sold FROM vehicle_units WHERE COALESCE(transferred,false)=false AND status='sold'");
            const inventoryStatus = { available, sold };
            const inventoryPie = [
            ...
            const itemsWithSRP = yield prisma_1.default.items.findMany({
                where: {
                    srp: {
                        not: null
                    }
            ...
            const branchCount = yield prisma_1.default.branches.count();
            ...
            sales.forEach((sale) => {
                ...
                if (ageGroup && ageGroups[ageGroup]) {
                    ...
                    (_a = sale.sales_items) === null || _a === void 0 ? void 0 : _a.forEach((item) => {
                        var _a;
                        const model = (_a = item.items) === null || _a === void 0 ? void 0 : _a.model;
                        if (model) {
            ...
            const dealershipSales = sales.reduce((sum, s) => {
            ...
            res.json({
                ...
                totalInventoryUnits: ((_a = (yield prisma_1.default.$queryRawUnsafe("SELECT COUNT(*)::int AS total FROM vehicle_units WHERE COALESCE(transferred,false)=false"))[0]) === null || _a === void 0 ? void 0 : _a.total) || 0
            });
        }
        catch (error) {
